...
# ----------------------------
# Helper: compute weighted eGene program score from an expression matrix
# - expr_mat: genes x samples (numeric)
# - gene_df: data.frame with gene symbol and logFC
# - gene_col: column name of gene symbols in gene_df
# - weight_col: column name of logFC in gene_df
# - logCPM: whether to transform counts to logCPM
# - use_abs_weight: if TRUE, use abs(logFC) as weights; otherwise use signed logFC
# ----------------------------
compute_program_score <- function(expr_mat,
                                  gene_df,
                                  gene_col = "gene",
                                  weight_col = "logFC",
                                  logCPM = TRUE) {
  stopifnot(gene_col %in% colnames(gene_df))
  stopifnot(weight_col %in% colnames(gene_df))

  gene_df <- gene_df[, c(gene_col, weight_col), drop = FALSE]
  colnames(gene_df) <- c("gene", "weight")
  gene_df <- gene_df[gene_df$gene %in% rownames(expr_mat), , drop = FALSE]

  if (nrow(gene_df) == 0) stop("No overlap between eGenes and expression matrix rownames.")

  if (logCPM) {
    expr_mat <- edgeR::cpm(expr_mat, log = TRUE, prior.count = 1)
  }

  X <- expr_mat[gene_df$gene, , drop = FALSE]   # genes x samples
  w <- gene_df$weight
  names(w) <- gene_df$gene

  # 加权平均
  score <- colSums(t(t(X) * w), na.rm = TRUE) / sum(abs(w), na.rm = TRUE)
  return(score)
}
...
# 0) Load eGene list
# ============================================================
eg_df <- read.csv("eGene-list.csv", row.names = 1) 
colnames(eg_df) <- c("gene", "logFC")                      
...
score_sc <- compute_program_score(
  expr_mat = pb_counts,
  gene_df = eg_df,
  gene_col = "gene",
  weight_col = "logFC",
